...
ECallout::ECallout(QChart *chart):
    QGraphicsItem(chart),
    m_chart(chart)
{
}

QRectF ECallout::boundingRect() const
{
...
void ECallout::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(option)
    Q_UNUSED(widget)
    QPainterPath path;
...
void ECallout::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    event->setAccepted(true);
}

void ECallout::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
...
void ECallout::setText(const QString &text)
{
    m_text = text;
...
void ECallout::setAnchor(QPointF point)
{
    m_anchor = point;
}

void ECallout::updateGeometry()
{
    prepareGeometryChange();
