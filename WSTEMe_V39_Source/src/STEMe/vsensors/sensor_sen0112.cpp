...
void Sensor_SEN0112::displayPoll()
{
    ...
    if(ui->checkBox_setMathAvg->isChecked()){
        _lux = VDevice::mathAvg(_lux,&m_VALUE_AVG,ui->spinBox_setSample->value());
        }
    ui->lcdNumber_Value->display(QString::number(_lux,'f',m_edev_sen0112->resolution()));
    //calculate basic parameters
    this->calculateParametes(_lux,m_edev_sen0112->resolution());
    //Chart graph
    if(m_vchartZoom){
        m_vchartZoom->vchartGraph(_lux,ui->comboBox_Temp_Unit->currentText());
...
void Sensor_SEN0112::on_pushButton_getSamples_clicked()
{
...
void Sensor_SEN0112::on_comboBox_Temp_Unit_currentIndexChanged(int index)
{
    switch(index){
...
void Sensor_SEN0112::on_spinBox_SampleTime_valueChanged(int arg1)
{
    if(arg1>ui->spinBox_SampleTime->minimum())ui->pushButton_SetSampleTime->setEnabled(true);
}

void Sensor_SEN0112::on_pushButton_SetSampleTime_clicked()
{
...
void Sensor_SEN0112::on_pushButton_Calib_clicked()
{
...
void Sensor_SEN0112::on_tabWidget_tabBarClicked(int index)
{
