#include "sensor_sen0110.h"
#include "ui_sensor_sen0110.h"

/////////////////////////////////////////////////////////////////////////////////////////////////Function for Sensor...

void Sensor_SEN0110::setDeviceWidget(VDevice dev,VUser user)
{
   ...
   //Create Sensor
   m_edev_sen0110 = new EDev_Sen0110();
   m_edev_sen0110->setSensorUnit(EDev_Sen0110::SSU_ppm);
   m_edev_sen0110->setSampleTime(m_modbusSampleTime);
   m_edev_sen0110->Start(dev);

   if(m_displayTimer)delete m_displayTimer;
   m_displayTimer = new QTimer();
   connect(m_displayTimer,&QTimer::timeout,this,&Sensor_SEN0110::on_displayTimer);
...
void Sensor_SEN0110::sensorCalibration(double valueCalib,double valueRaw)
{
    if(m_edev_sen0110->isConnect()){
        ...
        case VDeviceCalibration::Calib_Sampling_OnePoint:
            m_edev_sen0110->setSampleTime(m_deviceCalibration->sampleTime());
            break;
        ...
        case VDeviceCalibration::Calib_WriteData:
            m_edev_sen0110->writeCalib(m_deviceCalibration->pointCalib(),m_deviceCalibration->valueCalib(m_edev_sen0110->resolution()));
...
m_deviceCalibration->setConnectStatus(m_edev_sen0110->isConnect());

}

void Sensor_SEN0110::setInfoCalib()
{
    if(m_user.accountType()==VUser::root){
        int _p = m_edev_sen0110->pointCalib();
        if(_p>0){
...
void Sensor_SEN0110::sensorWriteReport(bool status)
{
    switch(m_SWS_Status){
    case Sensor_SEN0110::SWS_SetZero:
        break;
    case Sensor_SEN0110::SWS_Calib:
        ...
        break;
    case Sensor_SEN0110::SWS_default:
        return;
    }
    m_SWS_Status = Sensor_SEN0110::SWS_default;
}

void Sensor_SEN0110::calculateParametes(double value, int resolution)
{
        //Calculate
...
void Sensor_SEN0110::on_displayTimer()
{
    if(m_edev_sen0110->isConnect()){
        m_displayTimer->setInterval(m_displayInterval);
        m_edev_sen0110->setSampleTime(m_modbusSampleTime);
        m_edev_sen0110->update();
        switch (m_edev_sen0110->pollStatus()) {
        ...
        case ESensors::sensorReadReply:
            this->displayPoll();
            m_edev_sen0110->setPollStatus(ESensors::sensorIdle);
            break;
        ...
        m_edev_sen0110->setSampleTime(500);
        if(m_displayToggle){
    ...
    //Chart Calib
    if(m_deviceCalibration)this->sensorCalibration(m_edev_sen0110->valueCalib(),m_edev_sen0110->value());
}
void Sensor_SEN0110::displayPoll()
{
    double _co2 = m_edev_sen0110->valueCalib();
    //get average of value
    if(ui->checkBox_setMathAvg->isChecked()){
        _co2 = VDevice::mathAvg(_co2,&m_VALUE_AVG,ui->spinBox_setSample->value());
        }
    ui->lcdNumber_Value->display(QString::number(_co2,'f',m_edev_sen0110->resolution()));
    //calculate basic parameters
    this->calculateParametes(_co2,m_edev_sen0110->resolution());
    //Chart graph
    if(m_vchartZoom){
        m_vchartZoom->vchartGraph(_co2,ui->comboBox_Unit->currentText());
...
void Sensor_SEN0110::on_setChartParameter(int sampleTime)
{
...
Sensor_SEN0110::Sensor_SEN0110(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Sensor_SEN0110)
{
    ui->setupUi(this);


}
Sensor_SEN0110::~Sensor_SEN0110()
{
    delete ui;
}

void Sensor_SEN0110::closeEvent(QCloseEvent *event)
{
    QMessageBox  msg;
    ...
    msg.exec();
     if (msg.result() != QMessageBox::Yes) {
         ...
         m_edev_sen0110->close();
         event->accept();
     }
}

void Sensor_SEN0110::on_pushButton_Exit_clicked()
{
    this->close();
}

void Sensor_SEN0110::on_pushButton_ChartZoom_clicked()
{
    if(m_vchartZoom) delete m_vchartZoom;
        ...
        connect(m_vchartZoom,&Vchart_Zoom::setChartParameter,this,&Sensor_SEN0110::on_setChartParameter);
        m_vchartZoom->show();
}

void Sensor_SEN0110::on_pushButton_getSamples_clicked()
{
...
void Sensor_SEN0110::on_spinBox_SampleTime_valueChanged(int arg1)
{
    if(arg1>ui->spinBox_SampleTime->minimum())ui->pushButton_SetSampleTime->setEnabled(true);
}

void Sensor_SEN0110::on_pushButton_SetSampleTime_clicked()
{
...
void Sensor_SEN0110::on_pushButton_Calib_clicked()
{
    ...
    m_deviceCalibration->setResolution(m_edev_sen0110->resolution());
    ...
    m_deviceCalibration->setUnit(m_edev_sen0110->sensorUnitText());
    m_deviceCalibration->show();
}

void Sensor_SEN0110::on_tabWidget_tabBarClicked(int index)
{
    if(index==2)this->setInfoCalib();
}

void Sensor_SEN0110::on_comboBox_Unit_currentIndexChanged(int index)
{
    switch(index){
    case 0:m_edev_sen0110->setSensorUnit(EDev_Sen0110::SSU_ppm);
        break;
    case 1:m_edev_sen0110->setSensorUnit(EDev_Sen0110::SSU_mg);
        break;
    }
}
